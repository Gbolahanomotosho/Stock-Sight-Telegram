...
def normalize_ticker(ticker: str, provider: str) -> str:
    ...
    # MT4/MT5/Exness/OctaFX often use this format
    if ticker.isupper() and len(ticker) in (6, 7):  
        ...
        # Crypto like BTCUSD, ETHUSD
        if ticker.endswith("USD") and ticker[:3] in ("BTC", "ETH", "SOL", "ADA", "XRP"):
            symbol = ticker[:-3]
            if provider == "yahoo":
                return f"{symbol}-USD"
            if provider == "twelvedata":
    ...
    # ✅ TradingView format: often uses BINANCE:BTCUSDT, OANDA:EURUSD
    if ":" in ticker:
    ...
    # ✅ Fallback to static TICKER_MAPPING if defined
