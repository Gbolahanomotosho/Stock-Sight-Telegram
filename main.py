# main.py — Enhanced Telegram Service with Free Trial & Admin Notifications
# NEW FEATURES: 
# - Automatic 3-day free trial for new users
# - Admin notifications for new users and payments
# - Persistent subscription storage (survives restarts)
...
VERSION = "Stock Sight Telegram Service v2.0 (Enhanced)"
...
FREE_TRIAL_DAYS = 3  # Free trial period for new users
...
def _save_subs(subs: Dict[str, Any]) -> None:
    ...
    try:
        ...
        print(f"[INFO] Subscriptions saved to {SUBS_FILE}")
    except Exception as e:
...
def activate_subscription_for(user_id: int, days: int = 30, trial: bool = False) -> datetime.datetime:
...
# ----------------------------
# Core forecast function (PRESERVED FROM ORIGINAL)
# ----------------------------
...
def main():
    ...
    print(f"[INFO] Subscriptions file: {SUBS_FILE}")
